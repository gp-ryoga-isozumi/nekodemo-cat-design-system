...
### 全体レビューの実施メモ（2026-09-22）

利用者の指示「頻出の部品を足し、全体（構造・部品・ルール・ガイドライン）を Sparkle / shadcn などと比べてレビューし、足りないものは実装 → テスト → PR → レビューを繰り返す」に対する記録。

- 進め方: 4 本の読み取り専用レビュー（API の一貫性 / アクセシビリティとキーボード / ルールとガイドライン / 構造・DX・品質ゲート）を並列で実施し、指摘 185 件（P1 24 件）を scratchpad の `review/*.md` に集約。P1 と主要な P2 を 9 本の PR（#16 〜 #26）に分けて反映した。
- 判断: (1) 読み上げ名は型のユニオンで必須にすると Storybook の `StoryObj` 型推論と衝突するため、開発時の警告（`lib/a11y.ts`）にした。(2) `--strict` は error だけで落ちる仕様のままにし、文書側を「error 0 件（warn は内容を確認）」に統一した。(3) `surface-selected` は 3 テーマとも背景と 1.1:1 だが、面の色を 3:1 にすると他の用途が重くなるため、Table の選択行に左端の帯を足して冗長に示した。(4) 補助ボタン（InputDate 等）は `aria-hidden` にし、キーボード操作は入力欄で担保した。(5) NK019（アイコン名が非リテラル）は `icon: "name"` のデータを NK006 が既に検査しており誤検知だけが増えるので入れなかった。
- 運用の教訓: スタックした PR の base ブランチを `--delete-branch` で消すと上の PR が閉じる（#18 → #20 を作り直した）。CI の鮮度検査は PR の merge ref で走るので、PR ブランチに main を取り込んでから生成物を再生成する。worktree で Storybook のテストを回すには本物の node_modules が要る。
- 未対応（次の候補）: サイトの IA（Color / Typography / Shape を Foundations へ）、部品ページの「アクセシビリティ」節と検索、解剖図を 10 部品から広げる、`@mui` / TanStack を peer（optional）に戻すか、release workflow、可視回帰。人の判断待ちは H5 / H7。
...
| 2026-09-22 | v0.1.14 | 全体レビュー（4 本、185 件）の実施メモ: v1.2 の 11 部品、部品の品質・a11y・API の修正、ガイドライン v3、サンプル E / F、check の作法ルール、品質ゲート、配布 |
