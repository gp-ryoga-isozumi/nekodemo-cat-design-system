...
設計書 §15 の Phase 5 完成条件を、人が手元の AI コーディングツールで再現するための手順です。Claude Code では 2026-09-22 に条件 1〜3 に合格済み（条件 4 のテーマ切替はビルド CSS と配置の静的確認のみで、ブラウザでの実クリックは未実施。実装計画 Phase 5 の実施メモ）。Gemini CLI / Codex での実施（H6）はこの手順で行います。
