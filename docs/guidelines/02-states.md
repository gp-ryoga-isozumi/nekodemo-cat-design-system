...
| エラー | InlineMessage（negative）＋「再試行する」。全画面エラーにしない |
...
プロトタイプは擬似的な遅延を入れるので、短い待ちに読み込み表示を出すとちらつきます。次の 3 つを守ってください。

- 200ms 未満で終わる処理には読み込み表示を出しません（待ちが見えないので、出すと点滅して見えます）。
- いったん出したら 400ms は消しません（すぐ消えると「何か起きた」ようにしか見えません）。
- 部分更新（絞り込み・並び替え・ページ送り）では、画面全体ではなく該当する領域だけを読み込み表示に差し替えます。見出し・絞り込み行・ナビはそのまま残します。
...
- 権限が無くて見せられない: 「この案件を見る権限がありません」＋次にすること（「管理者に権限を依頼してください」）。主アクションは置きません
- エラーで表示できない: EmptyState ではなく InlineMessage（negative）＋「再試行する」を使います。0 件とエラーを同じ見た目にしません

EmptyState はマスコットを出してよい 4 か所のうちの 1 つです（[06-cat-flavor.md](./06-cat-flavor.md)）。表やフォームの内側には置きません。

## エラー

InlineMessage の `negative` を、失敗した領域のすぐ上に置き、「再試行する」のボタンを添えます。全画面エラーにはしません。エラーの詳細や再試行を Toast にだけ出すのも不可です。
...
- 200ms 未満なら読み込み表示を出さない。出したら 400ms は消さない。部分更新は該当領域だけを差し替える。
- 0 件は EmptyState を使い、初回は「まだ〜がありません」＋主アクション、検索結果 0 件は「条件に合う〜がありません」＋条件クリアにする。権限が無いときは主アクションを置かず、エラーは EmptyState にせず InlineMessage に出す。
- エラーは InlineMessage（negative）＋「再試行する」を該当箇所に出す。全画面エラーにしない。
