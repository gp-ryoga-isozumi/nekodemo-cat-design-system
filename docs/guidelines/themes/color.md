...
ロシアンブルーはダークスキームです。役割トークンだけで書いていれば文字と面のコントラストは保証されますが、次の 4 つは自分で面倒を見ます。

- **画像・スクリーンショットには枠を付ける**。`border border-border-low rounded-container` で囲みます。白い背景の画像がダークの面に直接載ると、画像の端が分からなくなります。
- **白背景の埋め込み要素は `bg-surface-card` で包む**。埋め込みの地図・iframe・外部のウィジェットは、周囲に `bg-surface-card` と余白を置いて「面の上に載っている」形にします。
- **`surface-overlay` を濃くしない**。オーバーレイはテーマ側で決まっています。`bg-black/60` のような自前の重ねを足すと、ダークでは背後が完全に潰れます。
- **影より border を併用する**。ダークでは `shadow-*` がほとんど見えないので、重なりは `border-border-low` と面の明度差で示します（[Shape](/guidelines/themes/shape/)）。

3 テーマを切り替えて崩れないことが完成の条件です。ライトで作ってからロシアンブルーで 1 度確認してください。
